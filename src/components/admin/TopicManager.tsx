...
export default function TopicManager({ topics, newTopic, setNewTopic, onAdd, onDelete, onUpdate }: TopicManagerProps) {
  ...
  return (
    <section className="glass p-6 sm:p-8 rounded-3xl border-white/[0.03] space-y-6 shadow-xl h-full flex flex-col">
      <h2 className="text-lg font-bold text-muted-foreground tracking-tight">{editingId ? 'Edit topic' : 'Manage topics'}</h2>
      <form 
      ...
      >
        <div className="grid grid-cols-2 gap-4">
           <input 
             ...
             className="w-full h-11 glass-input rounded-xl px-4 font-semibold text-sm text-foreground disabled:opacity-50 focus:ring-2 focus:ring-white/20 focus:outline-none"
           />
           <input 
             type="text" 
             required
             ...
             className="w-full h-11 glass-input rounded-xl px-4 font-semibold text-sm text-foreground focus:ring-2 focus:ring-white/20 focus:outline-none"
           />
        </div>
        <input 
          type="text" 
        ...
        <textarea 
          ...
          className="w-full h-24 glass-input rounded-xl p-4 font-semibold text-sm resize-none text-foreground focus:ring-2 focus:ring-white/20 focus:outline-none"
        />
        <input 
          type="text" 
          ...
          className="w-full h-11 glass-input rounded-xl px-4 font-semibold text-sm italic text-foreground focus:ring-2 focus:ring-white/20 focus:outline-none"
        />
      ...
      <div className="pt-6 border-t border-white/[0.02] space-y-4">
        <h3 className="text-[10px] font-bold tracking-widest text-muted-foreground text-center uppercase">Current topics</h3>
        <div className="grid grid-cols-1 gap-2">
          {topics.map(t => (
            <div key={t.id} className="p-3 glass rounded-xl border-white/[0.02] flex items-center justify-between group hover:border-white/10 transition-all focus-within:ring-2 focus-within:ring-white/20">
              ...
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
                <button onClick={() => startEdit(t)} className="p-1.5 hover:bg-white/10 rounded-lg text-muted-foreground hover:text-foreground transition-colors focus:ring-2 focus:ring-white/20 focus:outline-none">
                ...
                <button onClick={() => onDelete(t.id)} className="p-1.5 hover:bg-destructive/20 rounded-lg text-muted-foreground hover:text-destructive transition-colors focus:ring-2 focus:ring-destructive/20 focus:outline-none">
