...
export default function HomeClient({ initialTopics }: HomeClientProps) {
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      toast.danger("Failed to create room. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleJoinRoom = async () => {
    if (isLoading || !nickname || !roomCode) {
      toast.warning("Enter your name and the room code.");
      return;
    }
    ...
    try {
      ...
      toast.danger("Room not found or closed.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <main className="min-h-screen text-foreground flex flex-col items-center p-3 sm:p-6 md:p-10 page-transition overflow-y-auto relative z-10 selection:bg-white/20">
      ...
      <div className="absolute top-4 right-4 sm:top-6 sm:right-8 z-50">
        <Button 
          ...
          className="p-2 glass !border-white/5 hover:!border-white/20 transition-all group min-w-0 h-10 w-10 rounded-xl"
          ...
          <>
            ...
            <section className="w-full max-w-md mx-auto text-center">
              <Button 
                ...
                className="glass-button w-full h-14 rounded-2xl font-bold text-lg hover:bg-foreground hover:text-background transition-all bg-transparent"
              >
                Join a game
              </Button>
            </section>
